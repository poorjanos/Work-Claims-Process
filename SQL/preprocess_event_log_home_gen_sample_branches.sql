...
DROP TABLE t_claims_home_SAMPLE;
COMMIT;

/* Formatted on 2019. 04. 24. 10:11:05 (QP5 v5.115.810.9015) */
CREATE TABLE t_claims_home_SAMPLE
AS
   SELECT   e.f_szerz_azon,
    ...
    WHERE       e.f_karszam = b.f_karszam
            ...
            AND e.f_karbeido BETWEEN DATE '2017-01-01' AND DATE '2019-04-30'
            AND e.f_modkod LIKE '219%'
            AND b.f_vnem NOT IN ('1', '2', '30', '31', '32', '42', '55')
            AND e.f_karszam IN
                     ('9652437',
                      '10411922',
                      '10416305',
                      ...
                      '10421629',
                      '10412756',
                      '10423514');               --clean off unrelevant dangers (pet, accident and life)
COMMIT;


DROP TABLE t_claims_home_SAMPLE_distinct;
COMMIT;

CREATE TABLE t_claims_home_SAMPLE_distinct
AS
     SELECT   DISTINCT
              f_karszam,
              f_tszam,
...
DROP TABLE t_claims_home_SAMPLE_distinct_a;
CREATE TABLE t_claims_home_SAMPLE_distinct_a
AS
   SELECT   a.*, b.f_paid
     FROM   t_claims_home_SAMPLE_distinct a, (SELECT   DISTINCT f_paid, f_azon
...
DROP TABLE t_claims_home_SAMPLE_distinct_b;
CREATE TABLE t_claims_home_SAMPLE_distinct_b
AS
   SELECT   c.*, d.f_paid
     FROM   (SELECT   *
               FROM   t_claims_home_SAMPLE_distinct a
              WHERE   NOT EXISTS
                         (SELECT   1
                            FROM   t_claims_home_SAMPLE_distinct b
                           WHERE   f_tszam > 1 AND a.f_azon = b.f_azon)
                      AND f_azon NOT IN
                               (SELECT   f_azon FROM t_claims_home_SAMPLE_distinct_a))
            c,
...
DROP TABLE t_claims_home_SAMPLE_distinct_paid;

CREATE TABLE t_claims_home_SAMPLE_distinct_paid
AS
   SELECT   * FROM t_claims_home_SAMPLE_distinct_a
   UNION
   SELECT   * FROM t_claims_home_SAMPLE_distinct_b;
COMMIT;
...
CREATE TABLE T_claims_home_SAMPLE_MILESTONES
AS
   SELECT   f_paid AS case_id,
            ...
            TRUNC (f_utalas_first + 1) - 1 / (24 * 60 * 60)
               AS close_upperbound
     FROM   t_claims_home_SAMPLE_distinct_paid
...
SELECT   COUNT (DISTINCT f_paid)
 ...
 WHERE       EXISTS (SELECT   1
                       FROM   T_claims_home_SAMPLE_MILESTONES b
...
DROP TABLE t_claims_home_SAMPLE_kontakt_eventlog;
COMMIT;

CREATE TABLE t_claims_home_SAMPLE_kontakt_eventlog
AS
     SELECT   f_paid AS case_id,
              CASE
...
CREATE INDEX idx_mile
   ON T_claims_home_SAMPLE_MILESTONES (case_id);

CREATE INDEX idx_event
   ON t_claims_home_SAMPLE_kontakt_eventlog (case_id);
COMMIT;


ALTER TABLE t_claims_home_SAMPLE_kontakt_eventlog
ADD
(case_type varchar2(20)
);
COMMIT;

UPDATE   t_claims_home_SAMPLE_kontakt_eventlog a
   SET   case_type =
            (SELECT   case_type
               FROM   T_claims_home_SAMPLE_MILESTONES b
...
DROP TABLE t_claims_home_SAMPLE_eventlog;

CREATE TABLE t_claims_home_SAMPLE_eventlog
AS
     SELECT   a.*,
              b.f_mozgtip,
              c.f_mnev,
       ...
       FROM   t_claims_home_SAMPLE a,
              (SELECT   DISTINCT f_karszam, f_mozgtip, f_datum
                 FROM   ab_t_kar_osszes_mozgas where f_datum >= date '2017-10-01') b,
              ab_t_mozgas_kodok c,
              t_kar_mozg_en d
...
DROP TABLE t_claims_home_SAMPLE_pa_output;
CREATE TABLE t_claims_home_SAMPLE_pa_output
AS
   --Claim event date
   SELECT   f_karszam as case_id, 'Claim event occured' as activity, f_karido as event_end, f_branch as case_type
     FROM   t_claims_home_SAMPLE_eventlog
   UNION
   ...
   SELECT   f_karszam, 'Claim reported', f_karbeido, f_branch
     FROM   t_claims_home_SAMPLE_eventlog a
    WHERE   NOT EXISTS
               (SELECT   1
                  FROM   t_claims_home_SAMPLE_eventlog b
                 WHERE   f_mozgtip LIKE 'N1%' AND a.f_karszam = b.f_karszam)
   UNION
   --All other events
   SELECT   f_karszam, f_mozgtip || ' ' || f_mnev_eng, f_datum, f_branch
     FROM   t_claims_home_SAMPLE_eventlog

...
## Monthly Volume and Throughput Time of Reported Casco Claims
...
## Monthly Volume and Throughput Time By Process Branch
...
## Average Interactions per Claim File
...
## Average Interactions per Claim File By Process Branch
...
## Breakdown of Claims Handling Interactions by Department
...
t_interaction_counts_claimsdept <- t_eventlog %>%
  ...
  mutate(DEPT = case_when(DEPT == "Gepjarmu/szakertok" ~ "Car/Experts",
...
## Breakdown of Claims Handling Interactions by Department by Process Branch
...
### Contact Center Calls across Claims Lifecycle
...
### Contact Center Calls across Claims Lifecycle by Process Branch
...
### Distribution of Calls between Report and Close Dates

Most calls are received between report and closing date. Breaking down this period by days passed after report shows that most calls are received either right after report (between 0-5 days) or after 3 months (greater than 100 days).
...
t_calls_ccc <- t_eventlog %>%
  ...
  select(CASE_ID, CASE_TYPE, MILESTONES, ACTIVITY_EN, EVENT_END)
...
t_cutpoints %>%
  ...
  mutate(
    CALL_DIFF = ceiling(as.numeric(difftime(EVENT_END, REPORT_UPPERBOUND, units = "days"))),
    CALL_DIFF_CAT = cut(CALL_DIFF, c(seq(0, 200, 10), Inf)),
  ...
  geom_histogram(bins = 100) +
  coord_cartesian(xlim = c(0, 200)) +
  facet_grid(. ~ ACTIVITY_EN)
```

## Claims Handling

### Claims Handling Interactions across Claims Lifecycle
...
### Claims Handling Interactions across Claims Lifecycle by Process Branch

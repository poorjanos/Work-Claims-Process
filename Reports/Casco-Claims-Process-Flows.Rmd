...
t_cutpoints %>%
  ...
  inner_join(t_events_ccc, by = "CASE_ID") %>%
  ...
  group_by(CALL_DIFF_CAT, THROUGHPUT_TIME_CAT) %>%
  summarize(NUM_OF_CALLS = n()) %>%
  ungroup() %>%
  ggplot(aes(x = CALL_DIFF_CAT, y = NUM_OF_CALLS, group = 1)) +
  geom_line() +
  theme(axis.text.x = element_text(angle = 90, size = 6)) +
  facet_grid(. ~ THROUGHPUT_TIME_CAT)
```
